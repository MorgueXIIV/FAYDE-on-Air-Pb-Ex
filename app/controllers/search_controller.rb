		...
		if query.blank? then
			...
			if query.empty? then
				...
				if queryType=="1"
					...
					if actor.blank? then
						searchResults = Dialogue.includes(:actor, :alternates).searchTexts(query).first(maxSearchResults)
					else
						searchResults = Dialogue.includes(:actor, :alternates).searchTextsAct(query, actor).first(maxSearchResults)
